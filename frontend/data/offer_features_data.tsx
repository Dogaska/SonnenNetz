const project_features_data = [
  {
    title: "Project Description",
    text: "Embark on a transformative journey with our Neuperlach Green Energy Initiative, a groundbreaking opportunity for progressive investors eager to drive and benefit from sustainable energy practices. This project focuses on installing a comprehensive solar panel system within Neuperlach to supply renewable energy to the community, enhancing local energy independence and sustainability.",
  },
  {
    title: "Project Owner",
    text: "Maria Wagner",
  },
  { title: "Location", text: "81735, Neuperlach" },
  {
    title: "Budget Goal",
    text: "100.000 €",
  },
  {
    title: "Maximum Investment Limit",
    text: "10.000 Euros",
  },
  {
    title: "Surface Area Goal",
    text: "10.000 m2",
  },
  {
    title: "Installment Goal",
    text: "1.544 MW",
  },
  { title: "Start Date", text: "04.07.2024" },
  { title: "End Date", text: "31.10.2024" },
  {
    title: "Coummunity Type",
    text: "Local energy community",
  },
  {
    title: "Budget Progress",
    text: "90.000 € / 100.000 €",
  },
  {
    title: "Surface Area Progress",
    text: "10.000 / 10.000",
  },
];

const project_features_download_data = [
  {
    name: "Project Roadmap.pdf",
    url: "/project-roadmap",
    size: 2.4,
  },
  {
    name: "Project Statistics.docx",
    url: "/project-statistics",
    size: 5.6,
  },
];

const project_features_image_data = [
  {
    id: 1,
    url: "../../src/assets/images/neuperlach.png",
    text: "Description of image 1",
  },
  {
    id: 3,
    url: "../../src/assets/images/neuperlach3.png",
    alt_text: "Description of image 3",
  },
];

const surface_features_data = [
  {
    title: "Surface Description",
    text: "This surface belongs to a public school in the Neuperlach region. We want to support clean and local energy to be able to be part of zero emission city projects ...",
  },
  { title: "Location", text: "Neuperlach, Munich, Germany" },
  {
    title: "Effective Surface Area",
    text: "22.5 m2",
  },
  { title: "Surface Type", text: "Roof" },
  { title: "Start Date", text: "01.01.2024" },
  { title: "End Date", text: "01.01.2026" },
  {
    title: "Community Type",
    text: "Local energy community",
  },
];

const surface_features_download_data = [
  {
    name: "Surface Ownership Certificate.pdf",
    url: "/ownership-certificate",
    size: 2.4,
  },
];

const surface_features_image_data = [
  {
    id: 1,
    url: "https://tailwindui.com/img/ecommerce-images/product-feature-03-detail-01.jpg",
    text: "Description of image 1",
  },
  {
    id: 2,
    url: "https://tailwindui.com/img/ecommerce-images/product-feature-03-detail-02.jpg",
    alt_text: "Description of image 2",
  },
  {
    id: 3,
    url: "https://tailwindui.com/img/ecommerce-images/product-feature-03-detail-03.jpg",
    alt_text: "Description of image 3",
  },
];

const investment_features_data = [
  {
    title: "Investment Description",
    text: "This is a local project in Neuperlach region driven by  ...",
  },
  { title: "Location", text: "Neuperlach, Munich, Germany" },
  {
    title: "Coummunity Type",
    text: "Local energy community",
  },
  { title: "Start Date", text: "01.01.2024" },
  { title: "End Date", text: "01.01.2026" },
  {
    title: "Amount of Investment",
    text: "1000 Euros",
  },
];

const investment_features_download_data = [
  {
    name: "Bank Account Confirmation.pdf",
    url: "/projects/1/bank_account_confirmation",
    size: 2.6,
  },
];

const investment_features_image_data = [
  {
    id: 1,
    url: "https://tailwindui.com/img/ecommerce-images/product-feature-03-detail-01.jpg",
    text: "Description of image 1",
  },
  {
    id: 2,
    url: "https://tailwindui.com/img/ecommerce-images/product-feature-03-detail-02.jpg",
    alt_text: "Description of image 2",
  },
  {
    id: 3,
    url: "https://tailwindui.com/img/ecommerce-images/product-feature-03-detail-03.jpg",
    alt_text: "Description of image 3",
  },
];

export {
  project_features_data,
  project_features_image_data,
  project_features_download_data,
  surface_features_data,
  surface_features_image_data,
  surface_features_download_data,
  investment_features_data,
  investment_features_download_data,
  investment_features_image_data,
};
